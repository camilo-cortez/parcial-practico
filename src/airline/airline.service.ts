...
export class AirlineService {
    ...
    async create(airline: AirlineEntity): Promise<AirlineEntity> {
        const parsedDate = Date.parse(airline.founding_date);
        if (parsedDate > Date.now())
            throw new BusinessLogicException('The founding date cannot be a future date', BusinessError.PRECONDITION_FAILED);
